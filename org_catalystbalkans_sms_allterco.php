 ...
 *
 */
class org_catalystbalkans_sms_allterco extends CRM_SMS_Provider
{
    ...
    /**
     * Allterco API Server Session ID
     *
     * @var string
     */
     ...
     * Constructor
     *
     * Create and auth a Allterco session.
     *
     * @param array $provider
     * @param bool $skipAuth
     *
     * @return \org_catalystbalkans_sms_allterco
     */
 ...
 // initialize vars
	...
	if(isset($providerParams['provider'])){
        ...
        if (!isset(self::$_singleton[$cacheKey]) || $force) {
            ...
            self::$_singleton[$cacheKey] = new org_catalystbalkans_sms_allterco($provider, $skipAuth);
        }
        return self::$_singleton[$cacheKey];
    }

    /**
     * Authenticate to the Allterco API Server.
     *
    ...
    /**
     * Send an SMS Message via the Allterco API Server
     *
     * @param $recipients
     * @param $header
     * @param $message
...
$created_contact_id = $result_create_contact["id"];
	...
	$this->_fp = tmpfile();
        ...
        if (empty($response['http_code'])) {
            ...
            return PEAR::raiseError('Cannot connect to the Allterco API Server.');
        }
